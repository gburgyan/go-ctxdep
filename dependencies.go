...
func (d *DependencyContext) addDependenciesAndInitialize(ctx context.Context, deps ...any) {
	...
	d.setupInternalDependencies(ctx)
...
func (d *DependencyContext) setupInternalDependencies(ctx context.Context) {
	var logger GeneratorLogger
	err := d.FillDependency(ctx, &logger)
	if err == nil {
		d.logger = logger
	}
}
